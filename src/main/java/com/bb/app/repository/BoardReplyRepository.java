...
public interface BoardReplyRepository extends JpaRepository<BoardReplyEntity, Long> {
    ...
    @Query(value = "select * from board_reply_entity where member_id=:id", nativeQuery = true)
    public List<BoardReplyEntity> findAllByMemberId(@Param("id") Long id, Pageable pageable);
}
